...
/*   Updated: 2024/08/24 14:14:21 by moha             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	_env_empty(t_pdata data, u_padllst *dllst)
{
	...
	buf = NULL;
	_dllst_push_back(dllst, (void *)ft_strdup("OLDPWD"), ft_strdup(""), 0);
	data->args.old_pwd = (char *)(*dllst)->d_bot->addr_2;
	_dllst_push_back(dllst, (void *)ft_strdup("PWD"), (void *)getcwd(buf, 4096),
		0);
...
void	_env_filled(t_pdata data, u_padllst *dllst)
{
	...
	while (env[++i])
	{
		...
		if (sep == 1 && env[i][0] == '_')
			continue ;
		_dllst_push_back(dllst, (void *)ft_substr(env[i], 0, sep), (void *)ft_strdup(env[i] + (sep + 1)), 0);
		// _dllst_push_back(dllst, (void *)ft_substr(env[i], 0, sep), (void *)ft_substr(env[i], sep + 1, ft_strlen(env[i])), 0);
